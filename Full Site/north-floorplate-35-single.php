<?php
	$page_title = "North Building Floorplate 3-5 Single Tenant Fit Test :: Home Plate Seattle";
	$page_description = "";
	$fb_title = "";
	$fb_description = "";
	$section = "spaces";
	$subsection = "floorplate35";
	include('inc/header.php');
?>

<div role="main" id="content" class="clearfix">


	<div class="mainrow">
	<?php include('inc/sidebar-spaces.php'); ?>
	
	<section class="extended col clearfix">
		<div class="floorplate-info">
			<span class="fptitle">Total<br />Floor<br />Data</span>
			<span class="fpdata first">Gross Area<strong>29,584sf</strong></span>
			<span class="fpdata">Workstations<strong>153</strong></span>
			<span class="fpdata">Private Offices<strong>21</strong></span>
			<span class="fpdata">Conferences<strong>6</strong></span>
			<span class="fpdata last">Download<strong><a href="pdf/north-floor-35-single.pdf" target="_blank" class="pdf">PDF</a></strong></span>												
		</div>
		<h2 class="sm">Floors 3-5&nbsp; |&nbsp; Single-tenant Test Fit</h2>
		<img src="img/north-floor-35-single.png" alt="" />
		
	</section>
	<div class="clear"></div>
	</div><!--End Mainrow-->
	
</div><!--End #content-->
</div><!--End #wrap-->
  
 <?php include('inc/footer.php'); ?>